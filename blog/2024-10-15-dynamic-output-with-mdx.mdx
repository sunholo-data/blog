...
authors: me
tags: [agents, ux]
image: ./img/dynamic-ui-banner.png
...
It just so happens, that I had a prototype Chat React Component lying around as one of [Multivac's UI options](https://dev.sunholo.com/docs/multivac/#user-interfaces), and I can use it to stream custom GenAI APIs, so I'll attempt to host that Chat UI within this blog post, ask it to output MDX format, and then render them within the blog using MDX.
...
I use a Vertex deployed API on [Multivac](https://dev.sunholo.com/docs/multivac/) and make a new VAC called `dynamic_blog_mdx` which has no tools, just a prompt that asks it to output the React components.  I call to my own [Multivac](https://www.sunholo.com/ai-platform) cloud as this adds various features I want such as prompt management, analytics, configuration, user history etc. and runs custom code within a Cloud Run container.  
