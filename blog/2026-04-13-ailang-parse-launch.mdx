...
<TierComparison
  title="AILANG Parse vs the Competition"
  tiers={[
    {
      name: "AILANG Parse",
      color: "#4CAF50",
      strength: 100,
      strengthLabel: "10/10 features",
      icon: "🏆",
      description: "Direct XML extraction with full metadata support",
      points: [
        "Tables with merged cells across DOCX, PPTX, XLSX",
        "Track changes, comments, headers/footers",
        "Equations, field codes, text boxes, VML shapes",
        "Speaker notes, email threading, full ODF support"
      ],
      example: "docparse report.docx → structured output in milliseconds"
    },
    {
      name: "Pandoc",
      color: "#FF9800",
      strength: 30,
      strengthLabel: "3/10 features",
      icon: "📄",
      description: "Good format conversion but limited metadata extraction",
      points: [
        "Partial track changes support",
        "ODT only for ODF formats",
        "No equation, comment, or speaker note extraction"
      ],
      example: "pandoc report.docx -t markdown → loses tracked changes"
    },
    {
      name: "Kreuzberg",
      color: "#FF9800",
      strength: 20,
      strengthLabel: "2/10 features",
      icon: "🔍",
      description: "Partial header/footer and field code support",
      points: [
        "Partial headers/footers",
        "Partial field codes",
        "No track changes, comments, or ODF support"
      ],
      example: "Limited to basic DOCX text extraction"
    },
    {
      name: "Raw OOXML Parsers",
      color: "#F44336",
      strength: 30,
      strengthLabel: "3/10 features",
      icon: "⚙️",
      description: "Low-level XML access but no structured output",
      points: [
        "Raw merged cell data (unstructured)",
        "Partial comments (unstructured)",
        "Partial text boxes (unstructured)",
        "No ODF, email, or equation support"
      ],
      example: "Requires significant post-processing to be usable"
    }
  ]}
/>
...
<PullQuote color="#4CAF50">93.9% on OfficeDocBench v2 — tested against eight parsers on 69 files across 11 format variants and 7 metrics, with 100% format coverage versus the nearest competitor's 68%.</PullQuote>
...
<CodeCompare
  title="Document Parsing: Traditional vs AILANG Parse"
  left={{
    name: "Traditional Tools",
    color: "#F44336",
    badge: "Multi-step",
    code: `# Convert DOCX to PDF first\nlibreoffice --headless --convert-to pdf report.docx\n\n# Then run AI extraction on the PDF\npython -m unstructured.partition.pdf report.pdf\n\n# Track changes? Comments? Lost.\n# Headers/footers? Gone.\n# Merged table cells? Flattened.`,
    caption: "Lossy conversion pipeline — metadata destroyed before extraction begins"
  }}
  right={{
    name: "AILANG Parse",
    color: "#4CAF50",
    badge: "One command",
    code: `# Direct XML extraction — no conversion\ndocparse report.docx\n\n# Everything preserved:\n#   Track changes, comments,\n#   headers/footers, merged cells,\n#   equations, field codes\n\n# AI only when needed (PDFs, images)\ndocparse scan.pdf --ai gemini-3-flash-preview`,
    caption: "Deterministic extraction — all metadata preserved, AI only where needed"
  }}
/>
...
<SharedTopic topic="documents" current="/blog/ailang-parse-launch" />
{/* TOPIC_RESOURCES_END */}
