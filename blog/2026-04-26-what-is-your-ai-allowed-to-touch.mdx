...
authors: me
tags: [ai-delegation, capabilities, security, ailang]
image: ./img/screenshot_jason_lemkin_x_thread_replit_3.webp
slug: /what-is-your-ai-allowed-to-touch
description: "Assumed authority scales until it fails catastrophically. Declared authority fails small."
---
...
![Jason Lemkin's X thread describing Replit's AI agent deleting his production database](./img/screenshot_jason_lemkin_x_thread_replit_3.webp)

*Screenshot: [Jason Lemkin on X](https://x.com/jasonlk/status/1946069562723897802) describing the Replit incident*
...
<IncidentCard
  name="Replit deletes production database"
  year="2025"
  description="AI agent under explicit code freeze deleted live database, wiped 1,200+ user records, fabricated 4,000 fake replacements, then claimed rollback wasn't possible."
  principle="Declared Authority"
  principleNumber={1}
  outcome="Data recovered manually"
  color="#2196F3"
...
<IncidentCard
  name="Samsung bans ChatGPT"
  year="2023"
  description="Engineers pasted proprietary source code and internal meeting notes into ChatGPT, giving implicit permission for data to enter public training sets."
  principle="Declared Authority"
  principleNumber={1}
  outcome="Company-wide AI ban"
  color="#E91E63"
...
<ProtocolComparison
  title="Authority Failures — Three Incidents, One Missing Principle"
  mode="timeline"
  showLegend={true}
  items={[
    {
      ...
      year: "2022",
      color: "#4CAF50",
      description: "Chatbot told bereaved customer he could retroactively claim a bereavement discount. The policy didn't exist. Airline argued chatbot was a 'separate legal entity.'",
      features: ["Authority undefined", "No scope boundary on chatbot topics", "Output authority uncontrolled"],
      stats: { outcome: "Airline ordered to pay $812", principle: "1 — Declared Authority" },
      links: [{ title: "CBS News", url: "https://www.cbsnews.com/news/aircanada-chatbot-discount-customer/" }]
    },
    {
      name: "Samsung bans ChatGPT",
      year: "2023",
      color: "#E91E63",
      description: "Engineers pasted proprietary source code and meeting notes into ChatGPT. Data absorbed into public training sets.",
      features: ["Authority undefined", "Data leakage via human action", "No private/public boundary"],
      stats: { outcome: "Company-wide AI ban", principle: "1 — Declared Authority" },
      links: [{ title: "TechCrunch", url: "https://techcrunch.com/2023/05/02/samsung-bans-use-of-generative-ai-tools-like-chatgpt-after-april-internal-data-leak/" }]
    },
    {
      name: "Replit deletes production DB",
      year: "2025",
      color: "#2196F3",
      description: "AI agent under explicit code freeze deleted live database, wiped 1,200+ user records, fabricated 4,000 fake replacements.",
      features: ["Authority violated", "Prompt instruction ignored", "Fabricated rollback status"],
      stats: { outcome: "Data recovered manually", principle: "1 — Declared Authority" },
      links: [
        { title: "The Register", url: "https://www.theregister.com/2025/07/21/replit_saastr_vibe_coding_incident/" },
        { title: "Fortune", url: "https://fortune.com/2025/07/23/ai-coding-tool-replit-wiped-database-called-it-a-catastrophic-failure/" },
        { title: "Jason Lemkin's thread", url: "https://x.com/jasonlk/status/1946069562723897802" }
      ]
    },
  ]}
/>
...
<IncidentCard
  name="Air Canada chatbot invents policy"
  year="2022"
  description="Chatbot told bereaved customer he could retroactively claim a bereavement discount. Air Canada argued the chatbot was 'a separate legal entity.' Tribunal called this 'a remarkable submission.'"
  principle="Declared Authority"
  principleNumber={1}
  outcome="Airline ordered to pay $812"
  color="#4CAF50"
  link="https://www.cbsnews.com/news/aircanada-chatbot-discount-customer/"
  linkText="CBS News"
/>
...
<TierComparison
  title="Three Tiers of AI Capability Enforcement"
  tiers={[
    {
      ...
      icon: "⚠",
      description: "\"Don't delete the database.\" This is what the Replit customer had. A request, not a wall.",
      points: [
      ...
      example: "System prompt: \"Do not modify the production database under any circumstances.\"",
    },
    {
      ...
      strengthLabel: "Wall",
      icon: "◐",
      description: "Docker capabilities, IAM policies, read-only filesystems. Real walls, but coarse-grained.",
      points: [
        "Defined separately from the code",
        "Can't scope per-function or per-call",
        "Permission boundary wraps the whole application",
      ],
      example: "docker run --read-only --cap-drop=ALL --cap-add=NET_BIND_SERVICE",
    },
    {
      name: "Language-Level Capabilities",
      color: "#4CAF50",
      strength: 95,
      strengthLabel: "Steel",
      icon: "✓",
      description: "Every function declares its effects in the type signature. The permission document is the code.",
      points: [
        "Checked at compile time, enforced at runtime",
        "Per-function granularity with budgets",
        "No gap between permission and action",
      ],
      example: "func fetchData() -> string ! {Net @limit=5}",
    },
  ]}
/>
...
<CodeCompare
  title="Capability Declaration in Practice"
  left={{
    name: "Without AILANG",
    color: "#ef5350",
    badge: "Hope-based",
    code: "# IAM policy (lives in a separate JSON file)\n# Docker config (lives in a Dockerfile)\n# Prompt instruction (lives in a system message)\n# None are checked at compile time.\n# All can drift out of sync with the code.",
    caption: "Permission is defined somewhere else and enforced somewhere else.",
  }}
  right={{
    name: "AILANG",
    color: "#4CAF50",
    badge: "Compiler-enforced",
    code: "func fetchData() -> string ! {Net @limit=5} {\n  httpGet(\"https://api.example.com/data\")\n}\n\n-- Run: ailang run --caps Net --entry main app.ail\n-- Without --caps Net: hard refusal, not a warning",
    caption: "Permission is declared in the code and enforced by the runtime.",
  }}
/>
...
<SharedTopic topic="trust" current="/blog/what-is-your-ai-allowed-to-touch" />
{/* TOPIC_RESOURCES_END */}
