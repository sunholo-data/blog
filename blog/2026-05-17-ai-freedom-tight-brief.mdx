...
![Decision budget — vector visualisation showing entropy collapsing across five axes](./img/PLACEHOLDER-decision-budget-hero.webp)
...
![Three locations entropy can live when working with AI: prompt, code, operations — with cost increasing left-to-right](./img/PLACEHOLDER-three-locations-of-entropy.webp)
...
![The entropy cliff — AI delivery effort plotted against distance-from-training-prior. Flat for common requests (CRUD, auth, dashboards), then a steep cliff where domain specifics begin](./img/PLACEHOLDER-entropy-cliff.webp)
